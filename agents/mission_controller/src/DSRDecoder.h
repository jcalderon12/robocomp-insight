...
private:
  static std::tuple<uint64_t, std::string, std::string>
  split_header(const std::string &msg) {
  ...
  static void parse_modification(const std::string &content, DSREvent &mod) {
    ...
    if (mod.modification_type == DSRSpecialChars::MN ||
      ...
      if (mod.attributes.count(DSRAttributeNames::ID))
        mod.node_id =
            std::get<uint64_t>(mod.attributes[DSRAttributeNames::ID].value());

      // Get node type and node name
      if (mod.modification_type == DSRSpecialChars::MN) {
        if (mod.attributes.count(DSRAttributeNames::TYPE))
          mod.type = std::get<std::string>(
              mod.attributes[DSRAttributeNames::TYPE].value());
        if (mod.attributes.count(DSRAttributeNames::NODE_NAME))
          mod.node_name = std::get<std::string>(
              mod.attributes[DSRAttributeNames::NODE_NAME].value());
      }
    }
    ...
    // Edge common attributes
      ...
      if (mod.attributes.count(DSRAttributeNames::IDF))
        mod.edge_from_id =
            std::get<uint64_t>(mod.attributes[DSRAttributeNames::IDF].value());
      if (mod.attributes.count(DSRAttributeNames::IDT))
        mod.edge_to_id =
            std::get<uint64_t>(mod.attributes[DSRAttributeNames::IDT].value());
      if (mod.attributes.count(DSRAttributeNames::TYPE))
        mod.type = std::get<std::string>(
            mod.attributes[DSRAttributeNames::TYPE].value());
    }
  ...
  static void parse_keyframe(const std::string &content, DSREvent &mod) {
    ...
    std::string div = std::string(1, DSRSpecialChars::SLOT) +
                      std::string(1, DSRSpecialChars::K_DIV);
    size_t nodes_edges_div = content.find(div);

    if (nodes_edges_div == std::string::npos)
      throw std::runtime_error(
          "Invalid keyframe format: missing nodes/edges divisor");
  ...
  static void parse_nodes_section(const std::string &section,
                                  std::vector<DSR::Node> &nodes) {
    size_t pos = 0;
    while (pos < section.size()) {
      ...
      if (!item_str.empty() && item_str.size() > 0 &&
          item_str[0] != DSRSpecialChars::K_DIV) {
        ...
        if (!item_attrs.empty()) {
          ...
          if (item_attrs.count(DSRAttributeNames::ID))
            node.id(
                std::get<uint64_t>(item_attrs[DSRAttributeNames::ID].value()));
          if (item_attrs.count(DSRAttributeNames::NODE_NAME))
            node.name(std::get<std::string>(
                item_attrs[DSRAttributeNames::NODE_NAME].value()));
          if (item_attrs.count(DSRAttributeNames::TYPE))
            node.type(std::get<std::string>(
                item_attrs[DSRAttributeNames::TYPE].value()));

          for (const auto &[name, attr] : item_attrs) {
            if (name != DSRAttributeNames::ID &&
                name != DSRAttributeNames::NODE_NAME &&
      ...
      pos = next_hash + 1;
      if (next_hash >= section.size())
        break;
    }
  }

  static void parse_edges_section(const std::string &section,
    ...
    while (pos < section.size()) {
      ...
      if (!item_str.empty() && item_str.size() > 0 &&
        ...
        if (!item_attrs.empty()) {
          ...
          if (item_attrs.count(DSRAttributeNames::TYPE))
            edge.type(std::get<std::string>(
                item_attrs[DSRAttributeNames::TYPE].value()));
          if (item_attrs.count(DSRAttributeNames::IDF))
            edge.from(
                std::get<uint64_t>(item_attrs[DSRAttributeNames::IDF].value()));
          if (item_attrs.count(DSRAttributeNames::IDT))
            edge.to(
                std::get<uint64_t>(item_attrs[DSRAttributeNames::IDT].value()));
  ...
  static void parse_attributes(const std::string &attrs_str,
                               std::map<std::string, DSR::Attribute> &attrs) {
    size_t pos = 0;

    while (pos < attrs_str.size()) {
      while (pos < attrs_str.size() &&
             attrs_str[pos] == DSRSpecialChars::ATT_VAL)
        pos++;

      if (pos >= attrs_str.size())
        break;

      size_t name_div = attrs_str.find(DSRSpecialChars::ATT_NAME, pos);
      if (name_div == std::string::npos || name_div == pos)
        break;

      size_t type_div = attrs_str.find(DSRSpecialChars::ATT_TYPE, name_div);
      if (type_div == std::string::npos || type_div == pos)
        break;
      ...
      std::string type_code =
          attrs_str.substr(name_div + 1, type_div - name_div - 1);
  ...
  static DSR::ValType parse_value_by_code(const std::string &value_str,
